 ...
 * 
 * Requiere: OPENAI_API_KEY en archivo .env
 * Costo estimado: ~$3.60 USD (una sola vez)
 */
...
// Opciones a generar (id, prompt para DALL-E)
const PREVIEWS_TO_GENERATE = [
...
async function main() {
  ...
  const total = PREVIEWS_TO_GENERATE.length;
  const costPerImage = 0.04;
  const estimatedCost = total * costPerImage;
  
  console.log(`📊 Total de imágenes: ${total}`);
  console.log(`💰 Costo estimado: $${estimatedCost.toFixed(2)} USD\n`);
  console.log('Presiona Ctrl+C para cancelar...\n');
  
  await sleep(3000); // Dar tiempo para cancelar

  let generated = 0;
  let failed = 0;

  for (const preview of PREVIEWS_TO_GENERATE) {
    const filepath = path.join(OUTPUT_DIR, `${preview.id}.png`);
    
    // Saltar si ya existe
    if (fs.existsSync(filepath)) {
      console.log(`⏭️  [${generated + failed + 1}/${total}] ${preview.id} - Ya existe, saltando`);
      generated++;
      continue;
    }

    try {
      console.log(`🖼️  [${generated + failed + 1}/${total}] Generando: ${preview.id}...`);
      ...
      console.error(`❌ Error en ${preview.id}: ${error.message}`);
      failed++;
    }
  }
  ...
  console.log(`💰 Costo real: ~$${(generated * costPerImage).toFixed(2)} USD`);
}
